...
> **最終ゴール**: 07 章で、XSUAA と CIS の **メリット・デメリット・差分** が一目で見て取れる比較シートを作成します。単なる「用語の対応表」ではなく、各章で明らかになった **正直なトレードオフ**（例: [05 章](05-role-assignment-admin.md)で見た「複数アプリのポリシーを束ねる標準機能がない」「Admin Console でのポリシー作成は本流の運用ではない」「App-to-App 設定は純粋な追加コスト」など）も含めて整理する予定です。
