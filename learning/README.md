...
| 07 | [まとめ・移行チートシート](07-summary-migration-cheatsheet.md) | XSUAA と CIS の **メリット・デメリット・差分が一目でわかるシート**（概念対応表 ＋ 各章で明らかになったトレードオフの集約）・移行観点 |

> 本ガイドは段階的に作成しています。まずは **01 から** お読みください。

> **総まとめ**: [07 章](07-summary-migration-cheatsheet.md)に、XSUAA と CIS の **メリット・デメリット・差分** が一目で見て取れる比較シートがあります。単なる「用語の対応表」ではなく、各章で明らかになった **正直なトレードオフ**（例: [05 章](05-role-assignment-admin.md)で見た「複数アプリのポリシーを束ねる標準機能がない」「Admin Console でのポリシー作成は本流の運用ではない」「App-to-App 設定は純粋な追加コスト」など）も含めて整理しています。
