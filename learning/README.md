...
| 02 | 認証の違い *(作成予定)* | IAS = OIDC 発行者、証明書 / mTLS、トークン構造 |
| 03 | 認可の違い *(作成予定)* | scope/RBAC → DCL ポリシー・実行時 PDP・インスタンスベース認可 |
| 04 | 設定成果物の違い *(作成予定)* | `xs-security.json` → `identity` + `authorization.enabled` + DCL |
