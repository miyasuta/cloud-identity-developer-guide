...
| **認証トークン発行者** | XSUAA（CF 上の OAuth2 AS） | **IAS（OIDC Provider）** をアプリが直接信頼（[02](02-authentication.md)） | 標準 OIDC。跨サブアカウント・跨リージョンの信頼集約が容易 | アプリの信頼先・検証設定を IAS 前提に見直す必要 |
...
| **認可モデル** | scope / role-template による**静的 RBAC** | **DCL ポリシー**によるポリシーベース（ABAC 可）・**実行時 PDP 評価**（[03](03-authorization.md)） | 属性ベースの柔軟な条件、管理者が実行時に条件を派生できる | DCL という新しい記述言語と PDP/bundle の実行モデルを学ぶ必要 |
...
1. **複数アプリのポリシーを束ねる標準機能がない**（§3）
   個別 CAP が個別の `identity` インスタンス（＝AMS アプリ）を作ると、ポリシーもそのアプリに閉じます。束ねたければ[共有 IAS アプリ＋集中 DCL](shared-ias-app-central-dcl.md) という **構成変更を伴う回避策**が必要です。ロールコレクションの単純な代替にはなりません。
